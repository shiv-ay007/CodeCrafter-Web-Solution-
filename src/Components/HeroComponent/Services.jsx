...
const Services = () => {
  ...
  // Gap matches Tailwind's gap-2.5 (10px on mobile) and sm:gap-3 (12px on >=640px)
  const gap = visibleCount === 1 ? 10 : 12;
          ...
          >
            {extendedCards.map((item, index) => (
              ...
              >
                <Link
                  to={item.link}
                  className="group relative flex flex-col justify-between h-full min-h-[360px] sm:min-h-[385px] bg-white rounded-[24px] sm:rounded-[28px] p-6 sm:p-7 border border-slate-200/80 shadow-[0_4px_20px_-4px_rgba(2,43,50,0.05),0_1px_2px_rgba(0,0,0,0.03)] hover:shadow-[0_22px_45px_-12px_rgba(2,43,50,0.13),0_2px_6px_rgba(0,0,0,0.03)] hover:-translate-y-1.5 hover:border-[#022B32]/35 transition-all duration-300 overflow-hidden text-left"
                >
                  ...
                  <div>
                    ...
                    {/* Bold Service Title */}
                    <h3 className="text-[22px] sm:text-[24px] font-bold text-slate-950 tracking-tight mb-2.5 leading-snug group-hover:text-[#022B32] transition-colors">
                      {item.title}
                    </h3>

                    {/* Refined Description */}
                    <p className="text-[15px] sm:text-[16px] text-slate-600 font-normal leading-relaxed mb-4">
                      {item.description}
                    </p>
                  </div>

                  {/* Bottom Tag Pills Section with Ambient Glow */}
                  <div className="relative pt-4 mt-auto border-t border-slate-100/90">
                    ...
                    <div className="flex flex-wrap gap-2 relative z-10">
                      {item.tags.map((tag, tIdx) => (
                        <span
                          key={tIdx}
                          className="inline-flex items-center gap-1.5 px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-full border border-slate-200/90 bg-slate-50/80 text-[#022B32] font-semibold text-[12.5px] sm:text-[13px] tracking-tight shadow-2xs group-hover:border-[#022B32]/35 group-hover:bg-white hover:!bg-[#022B32] hover:!text-white hover:!border-[#022B32] hover:scale-105 transition-all duration-200"
                        >
